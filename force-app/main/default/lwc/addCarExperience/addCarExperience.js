...
export default class AddCarExperience extends LightningElement {
    ...
    addExperience(){
        ...
        console.log('Add ID '+this.carId);
        console.log('Add experte i '+JSON.stringify(fields));

        const recordInput  = {apiName : EXPERIENCE_OBJECT.objectApiName, fields};
        console.log('Record Create : '+ JSON.stringify(recordInput));

        createRecord(recordInput).then( carExperience => {
            this.showToast('SUCCESS', 'Experience Record Updated', 'success');
            const carExperienceAdded = new CustomEvent('experienceadded');
            this.dispatchEvent(carExperienceAdded);
        }).catch( error => {
